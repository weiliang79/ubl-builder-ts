...
import { UdtCode, UdtDate, UdtIdentifier, UdtName, UdtText, UdtTime } from '../datatypes/udt';

const ParamsMap: IGenericKeyValue<ParamsMapValues> = {
  URI: { order: 1, attributeName: 'cbc:URI', max: 1, classRef: UdtIdentifier },
  documentHash: { order: 2, attributeName: 'cbc:DocumentHash', max: 1, classRef: UdtText },
  hashAlgorithmMethod: { order: 3, attributeName: 'cbc:HashAlgorithmMethod', max: 1, classRef: UdtText },
  expiryDate: { order: 4, attributeName: 'cbc:ExpiryDate', max: 1, classRef: UdtDate },
  expiryTime: { order: 5, attributeName: 'cbc:ExpiryTime', max: 1, classRef: UdtTime },
  mimeCode: { order: 6, attributeName: 'cbc:MimeCode', max: 1, classRef: UdtCode },
  formatCode: { order: 7, attributeName: 'cbc:FormatCode', max: 1, classRef: UdtCode },
  encodingCode: { order: 8, attributeName: 'cbc:EncodingCode', max: 1, classRef: UdtCode },
  characterSetCode: { order: 9, attributeName: 'cbc:CharacterSetCode', max: 1, classRef: UdtCode },
  fileName: { order: 10, attributeName: 'cbc:FileName', max: 1, classRef: UdtName },
  description: { order: 11, attributeName: 'cbc:Description', max: undefined, classRef: UdtText },
};
